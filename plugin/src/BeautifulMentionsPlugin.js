...
import { $createTextNode, $getSelection, $isNodeSelection, $isParagraphNode, $setSelection, BLUR_COMMAND, COMMAND_PRIORITY_LOW, COMMAND_PRIORITY_NORMAL, KEY_BACKSPACE_COMMAND, KEY_DOWN_COMMAND, KEY_SPACE_COMMAND, PASTE_COMMAND, SELECTION_CHANGE_COMMAND, TextNode, } from "lexical";
...
import { $findBeautifulMentionNodes, $getSelectionInfo, $selectEnd, DEFAULT_PUNCTUATION, LENGTH_LIMIT, PRE_TRIGGER_CHARS, TRIGGERS, VALID_CHARS, getCreatableProp, getMenuItemLimitProp, getTextContent, isWordChar, } from "./mention-utils";
...
export function BeautifulMentionsPlugin(props) {
    ...
    const triggers = useMemo(() => { var _a; return (_a = props.triggers) !== null && _a !== void 0 ? _a : Object.keys(items !== null && items !== void 0 ? items : {}); }, [props.triggers, items]);
    ...
    const selectedMenuIndexRef = useRef(null);
    ...
    const convertTextToMention = useCallback((refocusAfterInsert = false) => {
    ...
    useEffect(() => {
        ...
        return mergeRegister(editor.registerCommand(SELECTION_CHANGE_COMMAND, () => {
            ...
            restoreSelection();
            const inserted = $insertMentionAtSelection(triggers, punctuation, trigger, value, data, autoSpace);
            if (!focus) {
                $setSelection(null);
            }
            return inserted;
            ...
            return removed;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(RENAME_MENTIONS_COMMAND, ({ trigger, newValue, value, focus }) => $renameMention(trigger, newValue, value, focus), COMMAND_PRIORITY_LOW), editor.registerCommand(OPEN_MENTION_MENU_COMMAND, ({ trigger }) => {
            restoreSelection();
            return $insertTriggerAtSelection(triggers, punctuation, trigger, autoSpace);
    ...
    if (props.combobox) {
        return (_jsx(ComboboxPlugin, { options: options, loading: loading, onQueryChange: setQueryString, onSelectOption: handleSelectOption, onReset: () => {
                setTrigger(null);
